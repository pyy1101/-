...
export default function PrivacyPage() {
  return (
    <div className="max-w-3xl mx-auto px-4 py-10">
      <h1 className="text-3xl font-bold mb-6 text-[var(--color-text)]">隐私政策</h1>
      <p className="text-sm text-[var(--color-text-dim)] mb-8">最后更新日期：{new Date().toISOString().split("T")[0]}</p>

      <h2 className="text-xl font-semibold mt-8 mb-3 text-[var(--color-text)]">信息收集</h2>
      ...
      <h2 className="text-xl font-semibold mt-8 mb-3 text-[var(--color-text)]">Cookie 和广告</h2>
      <p className="text-[var(--color-text-dim)] leading-relaxed mb-4">
      ...
      <h2 className="text-xl font-semibold mt-8 mb-3 text-[var(--color-text)]">数据安全</h2>
      <p className="text-[var(--color-text-dim)] leading-relaxed mb-4">
        由于所有数据处理均在本地浏览器中进行，您的使用数据不会传输到我们的服务器，因此不存在数据泄露的风险。特别是图片压缩、JSON 格式化等涉及内容处理的工具，您的数据完全保留在本地设备上。
      </p>

      <h2 className="text-xl font-semibold mt-8 mb-3 text-[var(--color-text)]">联系我们</h2>
